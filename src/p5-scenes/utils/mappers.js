...
const p5 = import("../libraries/p5.min.js");
const mappers = {
  ...
  circular: function (value, min, max, start = min, end = max, fn, base = 1) {
    return constrain(
      base - Math.abs(mappers.fn(value, min, max, -1, 1, fn)),
      start,
      end
    );
  },
  ...
  lerpPoints: (from, to, amount, func = p5.Vector?.lerp) => {
    const maxLength = Math.max(from?.length, to?.length);
    const result = [];
    for (let i = 0; i < maxLength; i++) {
      if (from[i % from.length] && to[i % to.length]) {
        const lerpedVector = func(
          from[i % from.length],
          to[i % to.length],
          amount
        );
  ...
  fastLerpPoints: (from, to, amount, func = p5.Vector?.lerp) => {
    const longest = from;
    const shortest = to;
    ...
    return longest.map((point, index) => {
      ...
      return func(longest[index], shortest[~~targetIndex], amount);
    });
  },
  traceVectors(
    count,
    ...
    const vectorsList = Array(count)
      .fill(undefined)
      .map((_, index) => vectorsGenerator(index / (count - 1)));
